...
| --- | --- | --- | --- |
| 1 | `ch := make(chan int, 1)` | empty | — |
| 2 | `ch <- 1` | `[1]` (full) | — |
| 3 | `ch <- 2` | blocks (waiting) | 🟥 *blocked* |
| 4 | `<-ch` reads `1` | buffer empty | unblocks send |
| 5 | *(Automatically)* | `[2]` now in channel | 🟢 send completes |
...
- Goroutines run on OS threads managed by Go runtime, but don’t map 1:1 to threads.
They are much lighter — thousands of goroutines can run on a few threads.