...
- **Lost update:**  transaction T2 overwrite transaction T1.

- **Dirty Reads**: T1 update data in database, then it failed anything but the updates value are accessed with another transaction.

- **Unrepeatable Read**: 1 transaction read the same time but different value.

## 18.2. Binary Locks

- Pros: lock full item while doing something.

- Cons: can not allow write, do not use commerically.

## 18.3. Shared / exclusive locks

- Unlike binary locks, Read/Write locks may be set to 3 values: SHARED, EXCLUSIVE or
UNLOCKED.

- SHARED: is READ-LOCKED, EXCLUSIVE is WRITE-LOCKED

1. **READ-LOCKED:** 

A read lock means:

- Multiple users/processes can read the resource at the same time.

- Nobody can modify/write it while the read lock exists.

2. **WRITE-LOCKED:**

A write lock means:

- One process/user is modifying the resource.

- Others usually cannot read or write until the operation finishes.

3. **UNLOCKED:**

An unlocked resource means:

- No restrictions.

- Anyone can access it according to permissions.

## 18.4. Persimistic and Optimistic Locking

- Optimistic Locking: version = version + 1

- Persimistic Locking: is lock real

## 18.5. Concurency control

![Concurrency Control](/images/concurrency_control.png)

## 18.6. Lock in SQL Server
...
# 19. What store in metadata of mysql

MySQL keeps metadata mainly in:

- the Data Dictionary (modern MySQL).Information about databases/schemas:
...
- system tables inside the mysql 

# 20. InnoDB architecture

![InnoDB Architecture](/images/innodb_architecture.png)

# 21. Log in DBMS

## 21.1. Transaction Log (internal transaction)

![Transaction Log](/images/transaction_log.png)

A transaction log records changes made by transactions.

It stores operations like:
...
## 21.2. Binlog (high-level for user query)

A binlog in MySQL is different from the redo/WAL log.
...
## 21.4. Write ahead log

- Store the changes of query.
...
- Store the order of checkpoint when excecuting.